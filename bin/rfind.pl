...
sub Initial {
 ...
 for (`cat $dbfiles.fa`) {
  ...
  if ($meta{$id}{form} eq 'stdshort' and /^[A-Z]+[a-z]{3}$/) {
  } elsif ($meta{$id}{form} eq 'prmshort' and /^[A-Z]+([a-z]+)[A-Z]+$/) {
   @{$meta{$id}}{qw/ivsL ivsR/} = ($-[1], $+[1])
  } elsif ($meta{$id}{form} eq 'std' and /^[A-Z]+([a-z]+)[A-Z]+[a-z]{3}$/) {
   @{$meta{$id}}{qw/tagL tagR/} = ($-[1], $+[1])
  } elsif ($meta{$id}{form} eq 'prm' and /^[A-Z]+([a-z]+)[A-Z]+([a-z]+)[A-Z]+$/) {
   @{$meta{$id}}{qw/ivsL ivsR tagL tagR/} = ($-[1], $+[1], $-[2], $+[2])
  } elsif ($meta{$id}{form} eq 'int' and /^[A-Z]+([a-z]+)[A-Z]+([a-z]+)[A-Z]+[a-z]{3}$/) {
