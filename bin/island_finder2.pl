...
sub Check_intervals {
 ...
 while (<HITS>) {
  ...
  $$hit{qStart_genomic} = ($trnas{$trna}{L}, $trnas{$trna}{R})[0.5-0.5*$dir] + $dir * ($qStart -1);
  $$hit{dupe} = 0;
 ...
 for my $trna (keys %hits) {
  MAIN: for (keys %{$hits{$trna}}) {
   ...
   if ($overlap) {Reason('2. non-integrase protein gene core overlap'); next}
   if ($$hit{dna} eq $trnas{$trna}{dna} and $$hit{hitDir} != $$hit{dir}) {Reason('3. opposite tRNA and hit orientations on same contig'; next}
   $interval = "trna:$trna $trnas{$trna}{orient}$$hit1{qStart_genomic} frag:$$hit1{hitOrient}$$hit1{hitStart}";
   unless ($$hit1{int_type} or $$hit2{int_type}) {
    Reason('4. covers neither A nor J site'); next;
   }
   $interval .= ' dupe:' . (4-$dupe) . 'prime';
   my ($coord, $compos);
   my $start_to_site = $$hit{int_site} - $$hit{qStart};
   my ($proximal, $distal) = ($$hit{qStart_genomic}, $$hit{hitStart});
   $proximal += $$hit{dir} * $start_to_start; $distal += $$hit{hitDir} * $start_to_site
   ...
   if ($$hit{dna} eq $trnas{$trna}{dna}) {
    ...
    unless ($origin) {Reason('5. tRNA duplication on wrong side of linear DNA'); next}
    if ($origin == 1) {
    ...
    $origin = -1;
    $compos = "cross";
    my $trnaEnd = (1, $stats{$$hit{dna}}{len})[0.5-0.5*$$hit{dir}*$$hit{dupe}];
    my $hitEnd = ($stats{$$hit{dna}}{len}, 1)[0.5-0.5*$$hit{hitDir}*$$hit{dupe}];
    $coord = "$trnas{$trna}{dna}/$proximal-$trnaEnd+$$hit{dna}/$distal-$hitEnd";
    $island = ($proximal, $trnaEnd);
   }
   my $isleSize;
   unless ($compos eq "cross") {
    ...
    $islesSize = abs($1 - $2) + abs($3 - $4);
   }
   if ($isleSize <= $minSize) {Reason('6. too short'); next}
   die unless $coord =~ /^([^\/]+)\/(\d+)-(\d+)/;
   my %temp = {dna => $1, L => $2, R => $3};
   my %other;
   if ($compos ne "simple") {
    die unless $coord =~ /[^\/]+\/\d+-\d+\+([^\/]+)\/(\d+)-(\d+)/;
    %other = {dna => $1, L => $2, R => $3};
   ...
   unless (keys %$ints) {Reason('7. contains no integrase gene'); next}
   my ($Lseq, $Rseq) = getSeq($coord);
   my $id = scalar(@isles);
...
sub IntsWithin {
 ...
 if (ref($e) eq 'ARRAY') {
  for my $i (0 .. scalar @{$e} - 1) {
   my ($dna, $L, $R, $origin) = @{$$e[$i]}{qw/dna L R origin/};
   ($L, $R) = sort {$a <=> $b} $L, $R; ($L, $R) = ($R, $L) if $origin == 1;
   for my $int (@{$integrases{$dna}}) { # int must be over half within island
    my ($within, $midpt) = (0, ($$int{L}+$$int{R})/2);
    if ($origin < 0) {$within ++ if $midpt > $L && $midpt < $R}
    else             {$within ++ if $midpt > $L || $midpt < $R}
    %{$ints{$$int{id}}} = (dna => $dna, L => $$int{L}, R => $$int{R}, annot => $$int{annot}) if $within;
   }
  }
 } else {
  my ($dna, $L, $R, $origin) = @{$e}{qw/dna L R origin/};
  for my $int (@{$integrases{$dna}}) { # int must be over half within island
   my $within;
...
sub ResolveOverlaps {
 ...
 for my $i (0 .. $#isles - 1) { # Find winner for overlap from different tandem group
  ...
  for my $j ($i+1 .. $#isles) {
   ...
   next unless $isles[$i]{dna} eq $isles[$j]{dna};
...
sub Print_islands{ #This outputs the final results of the program (overlaps still unresolved)
 ...
 for (sort {$$a{L} <=> $$b{L} || $$a{R} <=> $$b{R}} @isles) {
  ...
  for my $attribute (qw/size group segment origin proximal distal isleLseq isleRseq delta_int foreign housekeep hypoth delta_GC dinuc intList intCoords pot_ints pot_frags name/) {
...
sub getSeq {
 my ($L, $R, $acc, $ori, $site, $dupe) = @_;
 if ($dupe == 1) { if ($ori eq '+') { $R ++ } else { $L -- } }
 else { if ($ori eq '+') { $L -- } else { $R ++ } }
 my $Lseq = `perl $path/collectSeq.pl -i $prefix.fa -e $acc -L $L -R $L -f 100 -s`;
 my $Rseq = `perl $path/collectSeq.pl -i $prefix.fa -e $acc -L $R -R $R -f 100 -s`;
