...
use File::Spec;
# todo: other gencodes, exclude curated pseudos, place in alignments, opp orient hard mask, insist on cod/acc hit
...
my (%meta, %inseqs, %insizes, $id, $seq, @regions, %serials, %t, %seen, %gencode, %cheats);
my ($round, $flank) = (1, 350);

Initial();
chdir $outdir;
open FINAL, ">rfind.gff" or die "Cannot open outfile rfind.gff\n";
mkdir 'rfind'; chdir 'rfind';
my @blastCmd = (qw/blastn -max_target_seqs 5000000 -dust no -soft_masking false -lcase_masking -db/, $dbfiles, qw/-outfmt 6/);  # Run3 wants an array of its command words!
my @mergeCmd = (qw/bedtools merge -s -c/, '4,6', qw/-o collapse -i stdin/);
for $round (1..10) {
 unless (-f "$db.$round.merge") {
  my ($bed, $gnm, @hits, @beds, @merges, @cheathit) = ('', '');
  for (keys %inseqs) {
   $gnm .= ">$_\n$inseqs{$_}\n";
   if ($round == 1 and $cheats{$_}) {push @beds, [split ("\t", $cheats{$_})]}  #; for (@{$beds[0]}) {warn "'$_'\n"} exit}
  }
...
sub Initial {
 ...
 for (`cat $dbfiles.fa`) {
  if (s/^>(\S+)//) {$id = $1; while (s/^ ([^=]+)=([^;]+);/ /) {$meta{$id}{$1} = $2} die "$id\n" unless $meta{$id}{form}; next}
  chomp;
  @{$meta{$id}}{qw/seq len/} = ($_, length($_));
  if ($meta{$id}{form} eq 'StandardNoCDS' and /^[A-Z]+[a-z]{3}$/) {
  } elsif ($meta{$id}{form} eq 'PermutedNoCDS' and /^[A-Z]+([a-z]+)[A-Z]+$/) {
   @{$meta{$id}}{qw/ivsL ivsR/} = ($-[1], $+[1])
  } elsif ($meta{$id}{form} eq 'Standard' and /^[A-Z]+([a-z]+)[A-Z]+[a-z]{3}$/) {
   @{$meta{$id}}{qw/tagL tagR/} = ($-[1], $+[1])
  } elsif ($meta{$id}{form} eq 'Permuted' and /^[A-Z]+([a-z]+)[A-Z]+([a-z]+)[A-Z]+$/) {
   @{$meta{$id}}{qw/ivsL ivsR tagL tagR/} = ($-[1], $+[1], $-[2], $+[2])
  } elsif ($meta{$id}{form} eq 'GpIintron' and /^[A-Z]+([a-z]+)[A-Z]+([a-z]+)[A-Z]+[a-z]{3}$/) {
   @{$meta{$id}}{qw/tagL tagR intL intR/} = ($-[1], $+[1], $-[2], $+[2])
  } else {die "Can't parse $id $meta{$id}{form} $_\n"}
  if ($meta{$id}{tagR}) {
   my $orf = substr $_, $meta{$id}{tagL}, $meta{$id}{tagR}-$meta{$id}{tagL};
   $meta{$id}{tag} = Translate(uc $orf);
   #die "$meta{$id}{tag} ne $tag for $id\n" unless $meta{$id}{tag} eq $tag
  }
 }
 my $cheatfile = $dbfiles; $cheatfile =~ s/[^\/]+$/cheats/; return unless -f $cheatfile;
 for (`cat $cheatfile`) {
  chomp; if (/^(\S+)/) {$id = $1; $cheats{$id} = $_}
 }
}
