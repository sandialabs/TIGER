...
sub Check_intervals{ # Determine whether blast hit is a tRNA fragment marking an island end 
 ...
 MAIN: while (<HITS>){ # Blast hits, query=tRNA, hits=candidate island end tRNA fragments
  ...
  my ($Lseq, $Rseq) = getSeq($island[0], $island[1], $dir, $int_site, $dupe);
  my $id = scalar(@isles);
...
sub Bias {
 my ($L, $R, $origin) = @_;
 if ($origin == -1) {system "perl $path/collectSeq.pl -i $in.fa -L $L -R $R > test.fa"}
 else {
  system "perl $path/collectSeq.pl -i $in.fa -L $L -R $stats{len} > test.fa";
  system "perl $path/collectSeq.pl -i $in.fa -L 1 -R $R >> test.fa";
 }
...
sub getSeq {
 my ($L, $R, $ori, $site, $dupe) = @_;
 if ($dupe == 1) { if ($ori eq '+') { $R ++ } else { $L -- } }
 else { if ($ori eq '+') { $L -- } else { $R ++ } }
 my $Lseq = `perl $path/collectSeq.pl -i $in.fa -L $L -R $L -f 100 -s`;
 my $Rseq = `perl $path/collectSeq.pl -i $in.fa -L $R -R $R -f 100 -s`;
