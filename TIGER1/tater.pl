...
sub FindGenes {
 ...
 my $locus = '--locustag ' . $nickname if $nickname;
 RunCommand("prokka --rfam --prefix protein --locustag $dna --gcode $gencode --kingdom $kingdom --cpus $cpu --rnammer --notrna --outdir ./ --force --quiet $locus ../$dna.fa", "protein.gff");
...
sub CollectGff { # Still in protein subdirectory
 ...
 for (@{ReadFile("protein.gff")}) {
  ...
  my $id = '';
  if (s/locus_tag=([^;]+);//) {$id = $1; $id =~ s/\S+_/${nickname}_/;}
  if ($id) {s/ID=[^;]+/ID=$id/} else {if (/ID=([^;]+)/) {$id = $1; $id =~ s/\S+_/${nickname}_/;} else {$serials{NoID} ++; $id = "NoID.$serials{NoID}"}}
  %{$gff{$id}} = (dna => $f[0], L => $f[3], R => $f[4], line => $_, pfam => '', annot => '');
  s/ID=[^;]+/ID=$id/;
  #warn "$id: $gff{$id}{line}\n";
 }
 for (@{ReadFile("protein.phage")}) {next unless /ID=([^;]+)/; my $id = $1; $id =~ s/\S+_/${nickname}_/; my $label = 'Y-Int'; $serials{$label} ++; $gff{$id}{annot} = "$label.$serials{$label}"}
 for (@{ReadFile("protein.xer")}) {next unless /ID=([^;]+)/; my $id = $1; $id =~ s/\S+_/${nickname}_/; my $label = 'Xer'; $serials{$label} ++; $gff{$id}{annot} = "$label.$serials{$label}"}
 for (@{ReadFile("protein.integrons")}) {next unless /ID=([^;]+)/; my $id = $1; $id =~ s/\S+_/${nickname}_/; my $label = 'Integron-Int'; $serials{$label} ++; $gff{$id}{annot} = "$label.$serials{$label}"}
 for (@{ReadFile("tnp.tbl")}) { # Hmmer table
  my @f = split /\s+/;
  next unless $f[2] and $tnpCutoffs{$f[2]} and $f[4] <= $tnpCutoffs{$f[2]};
  $f[0] =~ s/\S+_/${nickname}_/;
