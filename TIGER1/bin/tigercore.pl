...
my $gnmlen = `$cmd`; chomp $gnmlen;
print "command '$cmd' yielded $gnmlen\n";
#NOTE: remove next line of code  after testing
if (-e $outdir) { system("rm -rf $outdir") }
...
for my $side (qw/L R/) { # Prepare q1 queries; q1 blast
 ...
 unless (-f "q1$side.fa") {
  my ($dir, $range) = ('plus',  ($coord-$actualQlen1{$side}+1) . '-' . ($coord+50));
     ($dir, $range) = ('minus', ($coord-50) . '-' . ($coord+$actualQlen1{$side}-1)) if $side eq 'R';
...
for my $side (qw/L R/) { # Process q1 blast hits; run and process q2 blast
 ...
 RunCommand("blastn -db ../genome -query q2$side.fa -outfmt 6 | grep $entry | awk '\$4 >= $minhit' | awk '\$7 < " . ($maxcross+3) . "' | sort -k1,1 -k12,12nr > q2$side.blast", "q2$side.blast");
 ...
 for my $q1 (sort keys %q1hits) {#print "$q1, ", join(',', keys %{$q1hits{$q1}}), "\n"; next;
  ...
  if ($crossover >= $maxcross) {
   ...
   RunCommand("blastdbcmd -db ../genome -dbtype nucl -entry '$entry' -range $L-$R -strand $gnmstrand > overlap/$L-$R", "overlap/$L-$R");
   RunCommand("blastn -db ../genome -query overlap/$L-$R -outfmt 6 > overlap/$L-$R.genome", "overlap/$L-$R.genome");
  ...
  $distL = 1 if $distL < 1;
  $distR = 1 if $distR < 1;
  #warn "Query ends: $proxL, $proxR, $distL, $distR\n";
...
sub getSeq {
 ...
 if ($L > $R) {($L, $R) = ($R, $L)}
 warn "blastdbcmd -db $db -dbtype nucl -entry '$entry' -range $L-$R -strand $strand -outfmt %s\n";
