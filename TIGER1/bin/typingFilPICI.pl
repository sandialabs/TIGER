...
system "perl $dir/makejobsFilPICI.pl > jobsFilPICI"; 
system "bash jobsFilPICI"; 
system "perl $dir/transposaser.pl $study"; 
system "perl $dir/selfblast.pl isles.fa > isles.invrep";
system "cat Isles/*/FilPICIphage.txt > FilPICIphage.txt"; 
...
for (`cat $study`) {
 ...
 $i{type} = ''; 
   ...
   elsif (($i{phage} == '2' or $i{phage} == '3' or $i{phage} == '4' or $i{phage} == '5') and ($i{ice} == '1')){$i{type} = 'ICE1'}
   elsif (($i{phage} == '2' or $i{phage} == '3' or $i{phage} == '4' or $i{phage} == '5') and ($i{ice} == '2')){$i{type} = 'ICE2'}
   elsif ($i{phage} == '1'){$i{type} = 'Phage1'}
   elsif ($i{phage} == '2' or $i{phage} == '3'){$i{type} = 'Phage2'}
   elsif ($i{phage} == '4'){$i{type} = 'PhageFil'}
   elsif ($i{phage} == '5'){$i{type} = 'PICI'}
   elsif ($i{ice} == '1'){$i{type} = 'ICE1'}
   elsif ($i{ice} == '2'){$i{type} = 'ICE2'}
   else {$i{type} = 'other'}
 for (qw/target supp intsum intLongest invRep crossover type phage ice source OLL OLR ORL ORR splice_site splice_site_type trna_dupe tRNA_len qStart qEnd bit_score project
  context contextsum isleLseq unintSeq isleRseq gnm dirOnU OL OU OR division phylum order class family
 genus species org ints deltaside len deltaint foreign housekeep hypoth delta_GC dinuc overall logscore
 rejectees rejected_by tandem tandem_power tandem_pos tnps/) {
