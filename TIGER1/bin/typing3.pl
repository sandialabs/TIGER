...
die "Usage: perl $0 IslandFile NickName\n" unless @ARGV > 1;
my $study = $ARGV[0]; 
...
for (`cat $study`){
 chomp; 
 my @f = split "\t"; 
 my %n; while ($f[8] =~ s/^([^=;]+)=([^;]*);//g) {$n{$1} = $2};
 my $nick = $ARGV[1];
 $n{ID} = "$nick$n{ID}"; 
 push @outIsles, join("\t", @f[0..7]);
 if (defined $n{$_}) {$outIsles[-1] .= "$_=$n{$_};"} else {$outIsles[-1] .= "$_=;"}
 print join('', @outIsles); @outIsles = ();
}

system "(mkdir Isles)"; 
system "perl $dir/gff2fa.pl $study > isles.fa";
system "perl $dir/splitFa.pl";
system "perl $dir/makejobs.pl > jobs"; 
system "bash jobs"; 
